 ...
 */
public class DrawIncidenceCube implements GLEventListener,
		KeyListener {
	...
	private static float rotateAngleY = 0.0f; // rotational angle for y-axis in
											  // degree
	protected static float z = -5.0f; // z-location
	protected static float rotateSpeedX = 0.0f; // rotational speed for x-axis
	protected static float rotateSpeedY = 0.0f; // rotational speed for y-axis

	protected static float zIncrement = 0.5f; // for zoom in/out
	protected static float rotateSpeedXIncrement = 0.5f; // adjusting x rotational
														// speed
	protected static float rotateSpeedYIncrement = 0.5f; // adjusting y rotational
														// speed

	private float right;
	
	protected IncidenceCube cube = null;
	
	protected int n = 0;//dimension
	
	private float textScaleFactor;

	// Lighting
	protected static boolean lightOn = false;

	
	public DrawIncidenceCube(IncidenceCube cube) {
		this.cube = cube;
		this.n = cube.size();
		z = cube.getDrawingOptions().getZ();
	}
	...
	public void display(GLAutoDrawable drawable) {
		...
		//draws grid in blue
		DrawingOptions opts = this.cube.getDrawingOptions();
		if (opts.isShowGrid()) {
			for (int x = 0; x < n; x++)
				for (int y = 0; y < n; y++)
					for (int z = 0; z < n; z++) {
		...
		for (int x=0; x < n; x ++) {
			for (int y=0; y < n; y++) {
				int z = cube.plusOneZCoordOf(x, y);
	...
	protected void drawSquareLimits(GL2 gl) {
	...
	protected void selectCube(GL2 gl, float x, float y, float z) {
	...
	protected void drawCube1By1Solid(GL2 gl, float x, float y, float z, float red, float green, float blue) {
	...
	protected void pointMinus1(GL2 gl, float x, float y, float z) {
		this.drawCube1By1Solid(gl, x, y, z, 1, 0, 0);
	}
	
	protected void pointPlus1(GL2 gl, float x, float y, float z) {
		...
		if (this.cube.getDrawingOptions().isShowBoxes())
			this.drawCube1By1(gl, x, y, z);
	}
	
	protected void axisLetters(GL2 gl) {
	...
	protected void drawText(GL2 gl, String text, float x, float y, float z) {
	...
	protected void drawCube1By1(GL2 gl, float x, float y, float z) {
	...
	public void keyPressed(KeyEvent e) {
		switch (e.getKeyCode()) {
		case VK_SPACE: //move +/-1
			if (this.cube.proper()) {
				this.cube.moveFromProper();
			} else {
				this.cube.moveFromImproper();
			}
			break;
